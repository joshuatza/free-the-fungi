...
import { dancingScriptBold } from './dancingScript';
...
export function generateSignedLetter(params: LetterParams): string {
	...
	// ── Signature block ──
	// Ensure enough room for signature + CC + footer (~65mm)
	...
	// Cursive signature using Dancing Script
	...
	if (!isCull) {
		// Check if CC fits, otherwise new page
		if (y > pageH - 30) { doc.addPage(); y = 22; }
		...
		const ccs = [
			...
			'Mrs Michele Clarke, MP — Portfolio Committee on Health (mclarke@parliament.gov.za)',
			'Minister of Health',
			'Minister of Justice and Constitutional Development',
		];
		for (const cc of ccs) {
			doc.text(cc, margin + 6, y);
			y += 4;
		}
	}

	// Footer — always on last page, positioned from bottom with enough clearance
	...
	doc.text('Generated by Free The Fungi — https://free-the-fungi.pages.dev', margin, footerY);
